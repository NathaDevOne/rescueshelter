...
exports.allUsers = async (req, res, next) => {
    const users = await User.find({role:['Admin','Employee','Volunteer']});
    res.status(200).json({
        success: true,
        users
    })
...
exports.allAdopters = async (req, res, next) => {
...
exports.loginUser = async (req, res, next) => {
    ...
    if (user.status === 'Inactive') {
        return next(new ErrorHandler('Your account is currently deactivated', 401));
    }
...
exports.forgotPassword = async (req, res, next) => {
    ...
    try {
        ...
        await sendEmail({
            email: user.email,
            subject: 'Animal Shelter Password Recovery',
            message,
            html
        })
...
exports.activateAdopter = async (req, res, next) => {
    let user = await User.findById(req.params.id)
